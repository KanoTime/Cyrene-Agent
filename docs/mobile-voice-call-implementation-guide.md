...
> 维护日期：2026-07-29
> 适用版本：Cyrene Voice `1.0.15`（Android `versionCode 16`）及同日桌面端代码
...
- 手机可创建、重命名、删除和分页选择当前角色的语音对话；正文只保存在桌面角色状态目录，退出应用后重新呼叫仍可续聊；
...
- 通话中可在 Android 暴露的蓝牙、耳机、扬声器和听筒输出之间切换，用于处理设备路由或快速对比经典蓝牙通话链路的音质；
...
| `mobile/src/voice-conversation-picker.tsx` | 新建、重命名、删除、分页浏览和选择语音历史 |
| `mobile/src/audio-output-routing.ts` | 推断 Android 默认输出并提供蓝牙/扬声器优先的切换顺序 |
...
Catalog 每页 12 条，完整正文不会经过 LiveKit data packet。手机退出或通话结束不删除桌面历史；下次呼叫重新选择同一名称即可续聊。切换角色后只看到新角色自己的目录。用户可删除未在当前通话中使用的历史；删除需要二次确认，桌面会同时原子更新目录索引与正文文件。当前活动历史拒绝删除，必须先结束通话。
...
- Android 真机；当前已验证部署要求手机在 5G 和 Wi-Fi 下都开启 VPN，以访问 Cloudflare、LiveKit 与 Expo 下载地址。
...
公开仓库的 `mobile/app.json` 不绑定维护者的 EAS Project。第一次构建时：

1. 先为 Android 选择自己的唯一 package；如果要覆盖安装既有 Cyrene Voice，则必须保留 `com.cyrene.agent.voice` 且持有同一 keystore。
2. 执行 `npx eas-cli@latest init`，选择当前账号下的新项目，并确认生成的新 `projectId` 已写回 `app.json`。
3. 执行 `npx eas-cli@latest project:info`，确认 owner/slug/projectId 属于当前账号。
4. 再执行 preview build；首次构建按提示创建或导入 Android keystore。
...
1. 覆盖安装，不卸载旧版；
2. 验证已有配对仍有效；
3. 保持手机 VPN 和桌面在线；当前验证链路不支持关闭 VPN；
4. 连续对话至少 5 轮且持续超过 60 秒；
5. 每轮都验证：手机问题被识别、角色正确、语音完整、回答后恢复聆听；
6. 验证静音、主动挂断；
7. 保持 VPN 在线，只切换 Wi-Fi/移动网络，验证重连或明确终止，不出现永久假连接；
8. 试听无沙声、无明显失真，音色与桌面 TTS 一致。
9. 新建一个自定义名称的语音对话并完成两轮，挂断并彻底退出手机应用；
10. 重新呼叫，选择同一历史并询问上一轮内容，确认角色能继续上下文；
11. 重命名一段未使用的历史，再删除并重新进入列表，确认删除不会复现；确认当前活动历史无法删除；
12. 切换手动模式，未点击“开始说话”时用旁人语音测试不触发 ASR；点击后说话并提交，确认正常回答；
13. 蓝牙通话时在角色说话过程中切到扬声器，再切回蓝牙，确认路由切换有效并记录两种路由听感；
14. 篡改/明文 data packet 只在自动测试中验证，真机链路不得提供明文降级入口。
...
- Cloudflare Worker + Durable Object 保持默认生产入口；替换控制面必须重新经过安全、网络和真机验收。
...
### 为什么当前方案不使用 CloudBase？

当前 Cloudflare 控制面不依赖 CloudBase 资源点，且 Durable Object 很适合单
Owner 的串行状态。公开分支不携带已弃用的个人 CloudBase 部署代码；需要研究
替代控制面时，应重新基于自己的账号、费用与网络条件做独立验证。

### 手机必须挂 VPN 吗？

**当前部署必须。** 已完成真机验收的链路要求 Android 手机在 5G 和 Wi-Fi
下都保持 VPN 在线，关闭 VPN 后无法保证访问 Worker 与 LiveKit，因此不属于
当前支持范围。协议设计本身并不把 VPN 当作身份认证或媒体加密；VPN 只解决
当前网络可达性，设备配对、HTTPS 鉴权和 LiveKit E2EE 仍然必须独立成立。
...
经典蓝牙耳机启用麦克风后通常会进入 HFP/SCO 通话链路，而不是高音质 A2DP
媒体链路，双向通话带宽因而可能明显下降。这不是把 TTS WAV 降采样造成的。
通话页的输出按钮可以即时在蓝牙与扬声器间对比；若扬声器清晰而蓝牙变糊，
优先检查耳机是否支持并启用了 LE Audio。不要为了强行保留 A2DP 而关闭通话
音频模式，否则会牺牲耳机麦克风、回声消除或双向稳定性。
