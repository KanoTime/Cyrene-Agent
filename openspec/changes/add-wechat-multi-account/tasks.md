...
本文件只记录实施顺序和 GitHub ticket 映射；任务状态与讨论以 GitHub Issues 为唯一事实来源。

## 阻塞关系

1. 账号存储与旧版迁移
2. 多账号连接池与绑定者过滤（blocked by 1）
3. 对话身份隔离与公平调度（blocked by 2）
4. 多账号设置页与登录生命周期（blocked by 1, 2）
5. 显式出站路由、账号权限与渠道任务（blocked by 3）
6. 角色切换协调与端到端验收（blocked by 3, 4, 5）
